...
#-------------------------------------------------------------------------------
def evaluate_cplfw(model, device):
    ...
    tpr, fpr, accuracy, val, val_std, far, fp, fn = cplfw_evaluate(embeddings, issame_list, distance_metric = 1, subtract_mean = True)
...
def train(model, 
    ...
    with open(log_filename, "a") as log_file:
        ...
        for epoch in tqdm(range(epochs)):
            ...
            model_dir = "/test_cuda/models/model_"+"{}".format(epoch)+".pth"
            torch.save(model.state_dict(), model_dir)
            print("state_dict saved")
            
            resnet = InceptionResnetV1(classify=False).to(device)
        
            dic = torch.load(model_dir)
            ...
            cplfw_accuracy = evaluate_cplfw(resnet, device)
            ...
            embeddings = np.array([embeddings_dict[path] for path in path_list])
            tpr, fpr, accuracy, val, val_std, far, fp, fn = evaluate(embeddings, issame_list, distance_metric = 1, subtract_mean = True)
            ...
            print(
                ...
                f"lfw_accuracy: {lfw_accuracy:.5f} | "
                f"xqlfw_accuracy: {xqlfw_accuracy:.5f} | "
                f"cplfw_accuracy: {cplfw_accuracy:.5f}")
            
            with open("lfw.txt", 'a') as file:
                file.write(str(lfw_accuracy)+"\n")
            with open("xqlfw.txt", 'a') as file:
                file.write(str(xqlfw_accuracy)+"\n")
            with open("cplfw.txt", 'a') as file:
                file.write(str(cplfw_accuracy)+"\n")
